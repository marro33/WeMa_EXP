...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    //创建人脸探测器
//    CIDetector *detector = [self highAccuracyFaceDetector];
//    //获取人脸识别
//    NSArray *detectResult = [detector featuresInImage:image];
//    UIView *resultView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];


    //边缘检测功能
    if (self.isBorderDetectionEnabled)//开启
    {
...
- (CIDetector *)highAccuracyFaceDetector
{
    static CIDetector *detector = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^
                  {
                      detector = [CIDetector detectorOfType:CIFeatureTypeFace context:nil options:@{CIDetectorAccuracy : CIDetectorAccuracyHigh}];
                  });
    return detector;
}
